...
class simple_mosaic_top_t : public systematic_scheduler_t, public simulation_t
{
public:
    simple_mosaic_top_t(simif_t &simif,
...
simple_mosaic_top_t::simple_mosaic_top_t(simif_t &simif,
                                         widget_registry_t &registry,
                                         const std::vector<std::string> &args)
...
int simple_mosaic_top_t::simulation_run()
{
    int exit_code = 0;
...
// in this case our 'simple_mosaic_top_t'
std::unique_ptr<simulation_t>
create_simulation(simif_t &simif,
                  widget_registry_t &registry,
                  const std::vector<std::string> &args)
{
    return std::make_unique<simple_mosaic_top_t>(simif, registry, args);
}
